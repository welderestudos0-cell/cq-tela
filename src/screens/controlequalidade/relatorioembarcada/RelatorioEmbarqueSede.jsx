...
// Gera PDF a partir do mesmo HTML da previa para manter layout consistente no envio
...
const GENERAL_INFO_TAB_FLOW = [
  ...
  { key: 'previa_pdf', label: 'Prévia PDF' },
];
...
const CONTAINER_FOTO_VINCULO_CAPTIONS = {
  temp_polpa_variedade: 'Temperatura de polpa - pergunta 14',
  espelho_pallet_variedade: 'Espelho de pallet - pergunta 13',
  set_point_container: 'Set point do container - pergunta 5',
  foto_4_drenos: 'Foto dos 4 drenos - pergunta 3',
  foto_numeracao_interna: 'Numeracao interna - pergunta 8',
  foto_numeracao_externa: 'Numeracao externa - pergunta 8',
  foto_termografo: 'Termografo - perguntas 9 e 10',
  foto_container_lacrado: 'Container lacrado - pergunta 15',
  foto_lacre: 'Numero do lacre - pergunta 15',
};
const CONTAINER_FOTO_PERGUNTA_ORDER = {
  foto_4_drenos: 3,
  set_point_container: 5,
  foto_numeracao_interna: 8.1,
  foto_numeracao_externa: 8.2,
  foto_termografo: 9,
  espelho_pallet_variedade: 13,
  temp_polpa_variedade: 14,
  foto_container_lacrado: 15.1,
  foto_lacre: 15.2,
};

const getContainerFotoVinculoCaption = (fieldKey, fallback = '') =>
  CONTAINER_FOTO_VINCULO_CAPTIONS[String(fieldKey || '').trim()] || fallback;
const getContainerFotoPerguntaOrder = (fieldKey) => {
  const key = String(fieldKey || '').trim();
  return Number.isFinite(CONTAINER_FOTO_PERGUNTA_ORDER[key]) ? CONTAINER_FOTO_PERGUNTA_ORDER[key] : 999;
};
...
export default function RelatorioEmbarqueSede({ navigation }) {
  ...
  const [containersHistorico, setContainersHistorico] = useState([]);
  const [containersHistoricoLoading, setContainersHistoricoLoading] = useState(false);
  ...
  // ── Fotos locais do Imprimir/Maturação (só para PDF, não salva) ──
  const [imprimirMangaLocais, setImprimirMangaLocais] = useState({});

  // ── Subtelas dentro de Informações Gerais ──
  const [generalInfoTab, setGeneralInfoTab] = useState('informacoes');
  ...
  const [filtroPriorizacao, setFiltroPriorizacao] = useState('');
  const [filtroPallets, setFiltroPallets] = useState('');
  const [carregamentosStatus, setCarregamentosStatus] = useState({}); // { id: { color, hasData, isSaved } }
  ...
  const [modalContainerTermografo, setModalContainerTermografo] = useState('');
  const [modalContainerEtiqueta, setModalContainerEtiqueta] = useState('');
  ...
  // Auto-carrega fotos e checklist do servidor quando container é definido
  useEffect(() => {
    if (containerSelecionadoKey && carregamentos.length > 0) {
      ...
      carregarChecklistDoContainer(containerSelecionadoKey);
      return;
    }
    setImprimirContainerFotos({ loading: false, data: {} });
    setImprimirMangaFotos({ loading: false, data: {} });
  ...
  const validateBeforePdf = () => {
    const serverPhotoCount =
      Object.values(imprimirContainerFotos.data || {}).reduce((s, arr) => s + (arr?.length || 0), 0) +
      Object.values(imprimirMangaFotos.data || {}).reduce((s, arr) => s + (arr?.length || 0), 0);
    if (totalPhotos === 0 && serverPhotoCount === 0) {
  ...
  const gerarPdfTeste = async () => {
    ...
    try {
      ...
      const html = await gerarPreviaEmbarque({ silencioso: true });
      if (!html) throw new Error('HTML da previa indisponivel.');
      ...
      if (!pdfResult?.uri) throw new Error('Arquivo PDF de teste nao foi gerado.');
      ...
      if (await Sharing.isAvailableAsync()) {
        ...
        Alert.alert('PDF de teste gerado', 'Arquivo salvo no cache local.');
      }
    } catch (error) {
  ...
  const enviarRelatorio = async () => {
    ...
    try {
      ...
      try {
        ...
        pdfBase64 = String(pdfResult?.base64 || '').trim();
        if (!pdfBase64) throw new Error('PDF base64 vazio.');
      } catch (pdfError) {
        console.warn('[RE] Falha ao gerar PDF pelo layout da prévia:', pdfError?.message || pdfError);
        throw new Error('Nao foi possivel gerar o PDF com o layout da previa.');
      }

      let backendMessage = '';
      let backendSent = false;
      try {
        const enviarFotosParaFallback = !pdfBase64;
      ...
      Alert.alert('Erro', msg || 'Nao foi possivel enviar o relatorio.');
    } finally {
  ...
  const handleMainAction = async () => {
    ...
    if (step === 0) {
      const subFlow = ['informacoes', 'mangas', 'container', 'previa_pdf'];
  ...
  const filteredContainers = containersHistorico.filter((carreg) => {
    const q = containerSearch.trim().toLowerCase();
    if (!q) return true;
    return (
      String(carreg?.container || '').toLowerCase().includes(q) ||
      String(carreg?.apelido || '').toLowerCase().includes(q) ||
      String(carreg?.id || '').toLowerCase().includes(q) ||
      String(carreg?.motorista || '').toLowerCase().includes(q)
    );
  });
  ...
  // Carrega apenas containers do historico SQLite para o campo de Imprimir.
  const carregarContainersHistorico = async () => {
    setContainersHistoricoLoading(true);
    try {
      const res = await api.get('/priorizacao/historico', { timeout: 15000 });
      const raw = Array.isArray(res?.data?.data)
        ? res.data.data
        : (Array.isArray(res?.data) ? res.data : []);

      const historicoNormalizado = raw
        .map((item) => {
          const idNum = Number(item?.oc ?? item?.id);
          ...
          return {
            ...
            dataSaida: String(item?.dataSaida || item?.data_saida || '').trim(),
            safra: String(item?.safra || '').trim(),
            source: 'sqlite_historico',
            pallets: pallets.map((p) => ({
              palletId: p?.planpal ?? p?.palletId,
              oc: idNum,
              controle: p?.controle ?? null,
              variedade: p?.variedade || '',
              caixaDescricao: p?.caixa_descricao || p?.caixaDescricao || '',
              classProd: p?.classe_prod ?? p?.classProd,
              calibre: p?.calibre,
              etiqueta: p?.etiqueta || 'NC',
              temp1: p?.temperatura_1 != null ? String(p.temperatura_1) : (p?.temp1 || ''),
              temp2: p?.temperatura_2 != null ? String(p.temperatura_2) : (p?.temp2 || ''),
              qtdCaixas: Number(p?.qtd_caixas ?? p?.qtdCaixas ?? 0),
            })),
          };
        })
        .filter(Boolean)
      ...
      setContainersHistorico(historicoNormalizado);
    } catch (error) {
      console.warn('[RE] Falha ao carregar historico de containers:', error?.message);
      setContainersHistorico([]);
    } finally {
      setContainersHistoricoLoading(false);
    }
  };
  ...
  const obterControlesDoCarregamento = async (carreg) => {
    ...
    // Só chama Oracle se algum pallet não tem controle definido
    const faltaControle = pallets.some((p) => !String(p?.controle || '').trim());
    if (pallets.length > 0 && faltaControle) {
  ...
  const carregarFotosContainerImprimir = async (container) => {
    ...
    try {
      ...
      // Fotos gerais do container (set point, drenos, numeração, lacre...)
      let dataNormalizada = {};
      if (carreg.apelido) {
      ...
      // Maturação e Firmeza vêm da Análise de Produção (por_controle) pelos controles dos pallets
      const pallets = Array.isArray(carreg?.pallets) ? carreg.pallets : [];
      const controles = [...new Set(pallets.map((p) => String(p.controle || '').trim()).filter(Boolean))];

      // Busca maturação e firmeza pela Análise de Produção — só pelo controle, sem filtrar variedade
      if (controles.length > 0) {
        const resultados = await Promise.allSettled(
          controles.map((controle) =>
            api.get('/analise-frutos/fotos-por-controle', { params: { controle } })
          )
        );
        ...
        resultados.forEach((r) => {
          ...
          (data.maturacao || []).forEach((u) => u && maturacaoUrls.push(toAbsoluteApiUrl(baseUrl, u)));
          (data.firmeza  || []).forEach((u) => u && firmezaUrls.push(toAbsoluteApiUrl(baseUrl, u)));
        });

        if (maturacaoUrls.length) dataNormalizada.maturacao_variedade = maturacaoUrls.filter((u, i, a) => a.indexOf(u) === i);
        if (firmezaUrls.length)   dataNormalizada.firmeza_variedade   = firmezaUrls.filter((u, i, a) => a.indexOf(u) === i);
      }
  ...
  const carregarFotosMangaImprimir = async (container) => {
    ...
    try {
      ...
      // Se todos os pallets já têm controle definido, pula a chamada Oracle (/pallet-dados)
      const todosTemControle = pallets.every((p) => String(p?.controle || '').trim() !== '');
      const resultadosPallets = todosTemControle
        ? pallets.map((p) => ({ status: 'fulfilled', value: { data: [] } }))
      ...
      // Busca fotos de TODAS as variedades/controles do container
      const resultadosFotos = await Promise.allSettled(
        opcoes.map((op) =>
          api.get('/analise-frutos/fotos-por-controle', { params: { controle: op.controle } })
        )
      );
      ...
      resultadosFotos.forEach((r) => {
        if (r.status !== 'fulfilled') return;
        const data = r.value?.data || r.value || {};
        (data.maturacao || []).forEach((u) => u && maturityUrls.push(toAbsoluteApiUrl(baseUrl, u)));
        (data.firmeza   || []).forEach((u) => u && firmnessUrls.push(toAbsoluteApiUrl(baseUrl, u)));
      });

      // Busca temp_polpa_variedade do container (relatorio_embarque/container/...)
      let pulpTemperatureUrls = [];
      if (carreg.apelido) {
        try {
          const resPT = await api.get('/priorizacao-pallets/listar-fotos-container', { params: { apelido: carreg.apelido } });
          const campos = resPT?.campos || resPT?.data?.campos || {};
          const dataN = normalizePriorizacaoFotosByCampo(campos, baseUrl);
          pulpTemperatureUrls = dataN.temp_polpa_variedade || [];
        } catch {}
      }

      const acumulado = {
        appearance: [],
        pulp_temperature: pulpTemperatureUrls,
        maturity: maturityUrls.filter((u, i, a) => a.indexOf(u) === i),
        firmness: firmnessUrls.filter((u, i, a) => a.indexOf(u) === i),
      };
  ...
  const carregarServidorFotosInline = async () => {
    ...
    try {
      ...
      // Firmeza e maturação vêm SOMENTE da Análise de Produção (por_controle)
      if (controles.length > 0) {
        const pallets = Array.isArray(selectedCarregamento?.pallets) ? selectedCarregamento.pallets : [];
        const resultados = await Promise.allSettled(
          controles.map((controle) => {
            const pallet = pallets.find((p) => String(p.controle || '') === String(controle));
            const variedade = pallet?.variedade || '';
            return api.get('/analise-frutos/fotos-por-controle', {
              params: { controle, ...(variedade ? { variedade } : {}) },
            });
          })
        );
        ...
        resultados.forEach((resultado) => {
          if (resultado.status !== 'fulfilled') return;
          const data = resultado.value?.data || resultado.value || {};
          (data.maturacao || []).forEach((u) => u && maturacaoUrls.push(toAbsoluteApiUrl(baseUrl, u)));
          (data.firmeza || []).forEach((u) => u && firmezaUrls.push(toAbsoluteApiUrl(baseUrl, u)));
        });

        dataNormalizada.maturacao_variedade = maturacaoUrls.filter((u, i, a) => a.indexOf(u) === i);
        dataNormalizada.firmeza_variedade   = firmezaUrls.filter((u, i, a) => a.indexOf(u) === i);
      }
  ...
  const fetchCarregamentos = async () => {
    ...
    try {
      ...
      const [resERP, resSQLite, resHistorico] = await Promise.allSettled([
        api.get('/carregamentos', { timeout: 90000 }),
        api.get('/priorizacao/listar', { timeout: 15000 }),
        api.get('/priorizacao/historico', { timeout: 15000 }),
      ]);

      const raw = resERP.status === 'fulfilled'
        ? (Array.isArray(resERP.value) ? resERP.value : (Array.isArray(resERP.value?.data) ? resERP.value.data : []))
        : [];

      const savedList = resSQLite.status === 'fulfilled'
        ? (Array.isArray(resSQLite.value?.data?.data)
          ? resSQLite.value.data.data
          : (Array.isArray(resSQLite.value?.data) ? resSQLite.value.data : []))
        : [];

      const historicoList = resHistorico.status === 'fulfilled'
        ? (Array.isArray(resHistorico.value?.data?.data)
          ? resHistorico.value.data.data
          : (Array.isArray(resHistorico.value?.data) ? resHistorico.value.data : []))
        : [];
      ...
      // Mescla com priorizacoes salvas no SQLite (OC + container), mesmo se nao vier do ERP.
      savedList.forEach((saved) => {
        const ocRaw = saved?.oc ?? saved?.id;
        const id = Number(ocRaw);
        if (!Number.isFinite(id)) return;

        const containerSQLite = String(saved?.container || '').trim();
        const apelidoSQLite = String(saved?.apelido || '').trim();
        const dataSaidaSQLite = String(saved?.dataSaida || saved?.data_saida || '').trim();
        const motoristaSQLite = String(saved?.motorista || '').trim();
        const safraSQLite = String(saved?.safra || '').trim();

        if (!grouped[id]) {
          grouped[id] = {
            id,
            container: containerSQLite,
            apelido: apelidoSQLite,
            motorista: motoristaSQLite,
            dataSaida: dataSaidaSQLite,
            safra: safraSQLite,
            pallets: [],
          };
          return;
        ...
        if (!String(grouped[id].container || '').trim() && containerSQLite) grouped[id].container = containerSQLite;
        if (!String(grouped[id].apelido || '').trim() && apelidoSQLite) grouped[id].apelido = apelidoSQLite;
        if (!String(grouped[id].motorista || '').trim() && motoristaSQLite) grouped[id].motorista = motoristaSQLite;
        if (!String(grouped[id].dataSaida || '').trim() && dataSaidaSQLite) grouped[id].dataSaida = dataSaidaSQLite;
        if (!String(grouped[id].safra || '').trim() && safraSQLite) grouped[id].safra = safraSQLite;
      });

      // Mescla historico denormalizado de PDF para permitir selecionar containers antigos no Imprimir.
      historicoList.forEach((saved) => {
        const ocRaw = saved?.oc ?? saved?.id;
        const id = Number(ocRaw);
        if (!Number.isFinite(id)) return;

        const containerHist = String(saved?.container || '').trim();
        const apelidoHist = String(saved?.apelido || '').trim();
        const dataSaidaHist = String(saved?.dataSaida || saved?.data_saida || '').trim();
        const motoristaHist = String(saved?.motorista || '').trim();
        const safraHist = String(saved?.safra || '').trim();
        const palletsHist = Array.isArray(saved?.pallets) ? saved.pallets : [];

        if (!grouped[id]) {
          grouped[id] = {
            id,
            container: containerHist,
            apelido: apelidoHist,
            motorista: motoristaHist,
            dataSaida: dataSaidaHist,
            safra: safraHist,
            pallets: palletsHist.map((p) => ({
              palletId: p?.planpal ?? p?.palletId,
              oc: id,
              controle: p?.controle ?? null,
              variedade: p?.variedade || '',
        ...
        if (!String(grouped[id].container || '').trim() && containerHist) grouped[id].container = containerHist;
        if (!String(grouped[id].apelido || '').trim() && apelidoHist) grouped[id].apelido = apelidoHist;
        if (!String(grouped[id].motorista || '').trim() && motoristaHist) grouped[id].motorista = motoristaHist;
        if (!String(grouped[id].dataSaida || '').trim() && dataSaidaHist) grouped[id].dataSaida = dataSaidaHist;
        if (!String(grouped[id].safra || '').trim() && safraHist) grouped[id].safra = safraHist;
        if ((!Array.isArray(grouped[id].pallets) || grouped[id].pallets.length === 0) && palletsHist.length > 0) {
          grouped[id].pallets = palletsHist.map((p) => ({
            palletId: p?.planpal ?? p?.palletId,
            oc: id,
            controle: p?.controle ?? null,
            variedade: p?.variedade || '',
      ...
      const cached = await loadCarregamentosFromCache();
      if (cached && cached.length > 0) {
        setCarregamentos(cached);
        Alert.alert('Offline', 'Mostrando dados salvos anteriormente.');
  ...
  // Busca dados do container/carregamento pela OC digitada
  const buscarDadosPorOC = async (oc) => {
    try {
      if (!oc || oc.trim().length === 0) return;
      
      const res = await api.get('/carregamentos/por-oc', { 
        params: { oc: oc.trim() },
        timeout: 15000 
      });
      
      if (res?.data?.success && res.data?.data) {
        const carregamento = res.data.data;
        
        // Preencher automaticamente os campos relacionados
        setGeneralInfoField('container', carregamento.container || '');
        setGeneralInfoField('vessel', carregamento.apelido || '');
        setGeneralInfoField('loading', carregamento.data_saida || '');
        
        console.log('[OC] Dados preenchidos automaticamente:', {
          container: carregamento.container,
          vessel: carregamento.apelido,
          loading: carregamento.data_saida,
        });
      }
    } catch (error) {
      console.warn('[OC] Erro ao buscar dados:', error?.message);
      // Não mostra alerta de erro pois é uma busca silenciosa
    }
  };
  ...
  // Determina a cor de status para cada carregamento (laranja/azul/verde) — paralelo
  const updateCarregamentosStatus = async (carrList) => {
    const results = await Promise.allSettled(
      carrList.map((carreg) =>
        api.get('/priorizacao/buscar', { params: { oc: carreg.id } })
          .then((res) => ({ id: carreg.id, data: res?.data ?? null }))
          .catch(() => ({ id: carreg.id, data: null }))
      )
    );
    ...
    results.forEach((result) => {
      ...
      const { id, data: saved } = result.value;
      if (saved && saved.sincronizado) {
        statusMap[id] = { color: '#64d964', hasData: true, isSynced: true };   // Verde
      } else if (saved && (saved.pallets?.length > 0 || saved.checklist)) {
        statusMap[id] = { color: '#00def2', hasData: true, isSynced: false };  // Azul
      } else {
        statusMap[id] = { color: 'orange', hasData: false, isSynced: false };  // Laranja
      }
    });

    setCarregamentosStatus(statusMap);
  };

  useEffect(() => {
    if (carregamentos.length > 0) {
      updateCarregamentosStatus(carregamentos);
      // Salva carregamentos em cache
      cacheCarregamentos(carregamentos).catch(() => {});
    }
  }, [carregamentos]);

  // Salva carregamentos em cache
  const cacheCarregamentos = async (list) => {
    try {
  ...
  // Carrega carregamentos do cache
  const loadCarregamentosFromCache = async () => {
    try {
      const cached = await AsyncStorage.getItem(PRIORIZACAO_CARREGAMENTOS_CACHE_KEY);
      return cached ? JSON.parse(cached).data : null;
    } catch (e) {
      console.warn('[Cache] Falha ao carregar carregamentos:', e?.message);
      return null;
    }
  };

  // Salva dados do modal em cache (para abrir sem internet)
  const cacheModalData = async (carreg) => {
    try {
      const modalData = {
        carregamento: carreg,
      ...
      await AsyncStorage.setItem(
        `${PRIORIZACAO_MODAL_CACHE_KEY}${carreg.id}`,
        JSON.stringify(modalData)
      );
    } catch (e) {
      console.warn('[Cache] Falha ao salvar modal data:', e?.message);
    }
  };

  // Carrega dados do modal do cache
  const loadModalDataFromCache = async (carregId) => {
    try {
      const cached = await AsyncStorage.getItem(`${PRIORIZACAO_MODAL_CACHE_KEY}${carregId}`);
      return cached ? JSON.parse(cached) : null;
    } catch (e) {
      console.warn('[Cache] Falha ao carregar modal data:', e?.message);
      return null;
    }
  };
  ...
  const openCarregamentoModal = async (carreg) => {
    ...
    let savedPalletMap = {};
    let savedPriorizacao = null;
    let savedPalletsList = [];
    let savedChecklist = null;
    try {
      const res = await api.get('/priorizacao/buscar', { params: { oc: carreg.id } });
      // interceptor retorna response.data direto → res = { success, data: { priorizacao, pallets, checklist } }
      const saved = res?.data ?? null;
      if (saved?.priorizacao) {
        savedPriorizacao = saved.priorizacao;
      }
      if (saved?.pallets?.length > 0) {
        savedPalletsList = saved.pallets;
    ...
    const carregNovo = {
      ...carreg,
      container: savedPriorizacao?.container || carreg.container || '',
      apelido: savedPriorizacao?.apelido || carreg.apelido || '',
      motorista: savedPriorizacao?.motorista || carreg.motorista || '',
      dataSaida: savedPriorizacao?.data_saida || carreg.dataSaida || '',
      safra: savedPriorizacao?.safra || carreg.safra || '',
      pallets: carreg.pallets.map((p) => {
        // Banco tem prioridade — dados confirmados
    ...
    // Se a OC nao vier do ERP com pallets, usa os pallets salvos no SQLite.
    if (carregNovo.pallets.length === 0 && savedPalletsList.length > 0) {
      carregNovo.pallets = savedPalletsList.map((sp) => ({
        palletId: sp.planpal,
        oc: carreg.id,
        controle: sp.controle ?? null,
        variedade: sp.variedade || '',
        caixaDescricao: sp.caixa_descricao || '',
        classProd: sp.classe_prod,
        calibre: sp.calibre,
        etiqueta: sp.etiqueta || 'NC',
        temp1: sp.temperatura_1 != null ? String(sp.temperatura_1) : '',
        temp2: sp.temperatura_2 != null ? String(sp.temperatura_2) : '',
        qtdCaixas: Number(sp.qtd_caixas || 0),
      }));
    }
    ...
    setModalChecklist(
      CHECKLIST_CONTAINER_ITEMS.map((item) => ({
        key: item.key,
        label: item.label,
        value: savedChecklist?.[item.key] ?? (rascunhoLocal?.checklist?.find((r) => r.key === item.key)?.value) ?? null,
        ...(item.hasTemperatura ? { temperatura: savedChecklist?.[`${item.key}_temp`] ?? (rascunhoLocal?.checklist?.find((r) => r.key === item.key)?.temperatura) ?? '' } : {}),
  ...
  const pickModalFotoCamera = async (campoKey) => {
    try {
      ...
      if (!result.canceled && result.assets?.length) {
        const uri = result.assets[0]?.uri;
        if (uri) {
          setModalFotos((prev) => ({ ...prev, [campoKey]: [...(prev[campoKey] || []), uri] }));
          // Salva na galeria do celular
          try {
            const { status } = await MediaLibrary.requestPermissionsAsync();
            if (status === 'granted') await MediaLibrary.saveToLibraryAsync(uri);
          } catch (e) {
            console.warn('[Galeria] Falha ao salvar foto:', e?.message);
          }
        }
      }
    } catch (error) { Alert.alert('Erro', 'Não foi possível abrir a câmera.'); }
  ...
  // Salva rascunho no AsyncStorage local ao fechar (sem enviar para o servidor)
  const salvarRascunhoLocal = async (carreg) => {
    ...
    try {
      ...
      console.warn('[Rascunho] Falha ao salvar local:', e?.message);
    }
  };
  ...
  const enviarPriorizacao = async () => {
    ...
    try {
      ...
      // Salva checklist pelo número do container para usar no Imprimir
      if (selectedCarregamento.container && modalChecklist.length > 0) {
        try {
          await api.post('/carregamentos/checklist', {
            container: selectedCarregamento.container,
            checklist: modalChecklist,
          });
        } catch (e) {
          console.warn('[Checklist] Falha ao salvar por container:', e?.message);
        }
      }
      ...
      setFiltroPallets('');
      setSelectedCarregamento(null);
      // Atualiza o status do carregamento para verde (sincronizado)
      setCarregamentosStatus((prev) => ({
        ...prev,
        [selectedCarregamento.id]: { color: '#64d964', hasData: true, isSynced: true },
      }));
    } catch (error) {
  ...
  const aplicarContainerInformacoes = async (carreg) => {
    ...
    // Garante que o container selecionado exista na lista principal usada nas abas de Imprimir.
    setCarregamentos((prev) => {
      const chaveAtual = obterChaveCarregamento(carreg);
      if (prev.some((c) => obterChaveCarregamento(c) === chaveAtual)) return prev;
      return [carreg, ...prev];
    });
  ...
  const renderChecklist = () => {
    return (
      <View style={st.card}>
        ...
        {!carregamentosLoading && carregamentos.filter((c) => {
          const q = filtroPriorizacao.trim().toLowerCase();
          if (!q) return true;
          return (
          ...
          <Text style={pgSt.emptyText}>{filtroPriorizacao ? 'Nenhum resultado para o filtro.' : 'Nenhum carregamento encontrado.'}</Text>
        )}
        ...
        {carregamentos.filter((c) => {
          ...
          const statusInfo = carregamentosStatus[carreg.id] || { color: 'orange', hasData: false, isSynced: false };
          return (
            <TouchableOpacity
              key={carreg.id}
              style={pgSt.carregCard}
                ...
                {isLoadingThisOC
                  ...
                  : (
                    <>
                      ...
                      />
                      <MaterialIcons name="local-shipping" size={20} color={ORANGE} style={{ marginRight: 4 }} />
                    </>
                  )}
    ...
    <View style={st.card}>
      ...
      <View style={st.field}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            void carregarContainersHistorico();
            setContainerSearch('');
      ...
      <View style={st.field}>
        ...
        <TextInput
          ...
          onBlur={() => buscarDadosPorOC(generalInfo.oc)}
        />
      </View>
  ...
  const gerarPreviaEmbarque = async ({ silencioso = false } = {}) => {
    if (previaLoading && !silencioso) return null;
    try {
      if (!silencioso) {
      ...
      const esc = (v) => String(v ?? '-').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').trim() || '-';

      // Converte URI local para base64 data URI
      ...
      // Pré-carrega todas as fotos como base64
      const sectionsComBase64 = await Promise.all((pdfSections || []).map(async (sec) => ({
        ...sec,
        items: await Promise.all((sec.items || []).map(async (item) => ({
          ...item,
          photosB64: await Promise.all((item.photos || []).map(async (p) => ({
            src: await toBase64(p.uri),
      ...
      const mangaSections = sectionsComBase64.filter((s) => s.key !== 'container');
      const containerSection = sectionsComBase64.find((s) => s.key === 'container');
      ...
      try {
        ...
        logoDataUri = await toBase64(logoAsset.localUri || logoAsset.uri);
      } catch {}
      const logoMarkup = logoDataUri
        ? `<img src="${logoDataUri}" alt="AGRODAN">`
        : 'AGRODAN';
      ...
      const headerHtml = `
        <div class="header">
          ...
          <div class="title-area"><h1>QUALITY CONTROL REPORT</h1><span>Controle de Qualidade</span></div>
        </div>`;

      // Página 1 — info geral + checklist
      const checklistRows = (checklist || []).map((item, i) => {
        const val = item.value;
        const rawLabel = String(item.label || '');
        const cleanLabel = rawLabel.replace(/^(\s*\d+\s*[\.\)-]\s*)+/, '').trim();
        const isSimNao = rawLabel.includes('etileno') || rawLabel.includes('Ventilacao a 40');
        const okLabel = isSimNao ? 'S' : 'C';
        const noLabel = isSimNao ? 'N' : 'NC';
        const okMark = isSimNao ? (val === 'S' ? 'X' : ' ') : (val === 'C' ? 'X' : ' ');
        const noMark = isSimNao ? (val === 'N' ? 'X' : ' ') : (val === 'NC' ? 'X' : ' ');
        const opts = `<span class="opt-block">( ${okMark} ) ${okLabel}</span><span class="opt-block">( ${noMark} ) ${noLabel}</span>`;
        return `<div class="checklist-row">
          <div class="checklist-question"><span class="checklist-number">${i + 1}.</span><span>${esc(cleanLabel || rawLabel)}</span></div>
          <div class="checklist-options">${opts}</div>
        </div>`;
      }).join('');
      ...
      const page1 = `<div class="page">
        ...
        <div class="info-grid">
          <div class="info-item"><div class="info-label">Customer</div><div class="info-value">${esc(info.customer)}</div></div>
          <div class="info-item"><div class="info-label">Container</div><div class="info-value">${esc(info.container)}</div></div>
          <div class="info-item"><div class="info-label">Vessel</div><div class="info-value">${esc(info.vessel)}</div></div>
          <div class="info-item"><div class="info-label">Loading</div><div class="info-value">${esc(info.loading)}</div></div>
          <div class="info-item"><div class="info-label">ETD</div><div class="info-value">${esc(info.etd)}</div></div>
          <div class="info-item"><div class="info-label">ETA</div><div class="info-value">${esc(info.eta)}</div></div>
        </div>
        <div class="checklist-section">
          <div class="checklist-title-row"><div class="checklist-title">CHECKLIST DO CONTAINER</div><div class="checklist-line"></div></div>
          <div class="checklist-box">${checklistRows}</div>
        </div>
      </div>`;

      // Converte fotos locais da maturação para base64
      const mangaLocaisBase64 = {};
      for (const [key, uris] of Object.entries(imprimirMangaLocais)) {
        mangaLocaisBase64[key] = await Promise.all(uris.map((uri) => toBase64(uri)));
      }

      // Busca fotos por variedade a partir dos pallets da OC
      const baseUrlPreview = (api.getCurrentURL() || '').replace(/\/api$/, '');
      const carreg = containerSelecionadoKey ? buscarCarregamentoPorChave(containerSelecionadoKey) : null;
      const pallets = Array.isArray(carreg?.pallets) ? carreg.pallets : [];

      // Agrupa controles por variedade
      const variedadeMap = {};
      pallets.forEach((p) => {
        const vari = String(p.variedade || 'SEM VARIEDADE').trim().toUpperCase();
        const ctrl = String(p.controle || '').trim();
        if (!ctrl) return;
        if (!variedadeMap[vari]) variedadeMap[vari] = [];
        if (!variedadeMap[vari].includes(ctrl)) variedadeMap[vari].push(ctrl);
      });

      // Para cada variedade, busca fotos dos controles + temp polpa do container
      const containerDataPreview = imprimirContainerFotos.data || {};
      const pulpUrls = (containerDataPreview.temp_polpa_variedade || []);

      const variedadesFotos = await Promise.all(
        Object.entries(variedadeMap).map(async ([variedade, controles]) => {
          const matUrls = [], firmUrls = [];
          for (const controle of controles) {
            try {
              const res = await api.get('/analise-frutos/fotos-por-controle', { params: { controle } });
              const data = res?.data || res || {};
              (data.maturacao || []).forEach((u) => u && matUrls.push(toAbsoluteApiUrl(baseUrlPreview, u)));
              (data.firmeza   || []).forEach((u) => u && firmUrls.push(toAbsoluteApiUrl(baseUrlPreview, u)));
            } catch {}
          }
          return { variedade, matUrls, firmUrls };
        })
      );

      const fotoCard = (url, label) => url
        ? `<div class="photo-card"><img src="${esc(url)}" alt="${esc(label)}"><div class="photo-caption">${esc(label)}</div></div>`
        : `<div class="photo-card"><div class="photo-placeholder">${esc(label)}</div></div>`;

      // Gera uma página por variedade
      const mangaPages = variedadesFotos
        .filter((v) => v.matUrls.length > 0 || v.firmUrls.length > 0 || pulpUrls.length > 0)
        .map((v) => {
          const titulo = `MANGA ${v.variedade}`;
          const allFotos = [
            ...v.matUrls.map((u) => ({ u, l: 'Maturity' })),
            ...(mangaLocaisBase64.maturity || []).filter(Boolean).map((u) => ({ u, l: 'Maturity' })),
            ...v.firmUrls.map((u) => ({ u, l: 'Firmness' })),
            ...(mangaLocaisBase64.firmness || []).filter(Boolean).map((u) => ({ u, l: 'Firmness' })),
            ...pulpUrls.map((u) => ({ u, l: 'Pulp Temperature' })),
            ...(mangaLocaisBase64.pulp_temperature || []).filter(Boolean).map((u) => ({ u, l: 'Pulp Temperature' })),
          ];
          const grid = allFotos.map(({ u, l }) => fotoCard(u, l)).join('');
          return `<div class="page">
            <div class="section-title">${esc(titulo)}</div>
            <div class="section-subtitle">photos</div>
            <div class="photo-grid">${grid || '<div class="empty-note">Sem fotos</div>'}</div>
          </div>`;
        }).join('');

      // Página container — combina fotos locais + servidor
      const containerServerData = imprimirContainerFotos.data || {};
      const containerLocalPhotos = containerSection
        ? (containerSection.items || []).flatMap((i) => i.photosB64 || [])
        : [];
      const CAMPOS_MANGA = new Set(['maturacao_variedade', 'firmeza_variedade', 'temp_polpa_variedade']);
      const containerServerPhotos = Object.entries(containerServerData)
        .filter(([key]) => !CAMPOS_MANGA.has(key))
        .flatMap(([key, urls]) =>
          (urls || []).map((url) => ({ src: toAbsoluteApiUrl(baseUrlPreview, url), fieldKey: key }))
        )
        .filter(Boolean)
        .filter((p) => Boolean(p?.src));
      const allContainerPhotos = [...containerLocalPhotos, ...containerServerPhotos];
      const orderedContainerPhotos = [...allContainerPhotos].sort((a, b) => {
        const orderA = getContainerFotoPerguntaOrder(a?.fieldKey);
        const orderB = getContainerFotoPerguntaOrder(b?.fieldKey);
        if (orderA !== orderB) return orderA - orderB;
        return String(a?.fieldKey || '').localeCompare(String(b?.fieldKey || ''));
      });
      const containerGrid = orderedContainerPhotos.length
        ? orderedContainerPhotos.map((p, idx) => `<div class="photo-card">
            ${p.src ? `<img src="${esc(p.src)}" alt="photo ${idx+1}">` : `<div class="photo-placeholder">Foto ${idx+1}</div>`}
            <div class="photo-caption">${esc(getContainerFotoVinculoCaption(p.fieldKey, `photo ${idx + 1}`))}</div>
          </div>`).join('')
        : '<div class="empty-note">Sem fotos</div>';

      const containerPage = `<div class="page">
        <div class="section-title">Container</div>
        <div class="section-subtitle">photos</div>
        <div class="photo-grid photo-grid-container">${containerGrid}</div>
      </div>`;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=595, initial-scale=0.55, minimum-scale=0.3, maximum-scale=4, user-scalable=yes">
  <style>
    :root {
      --green-900: #0f4f30;
      --green-700: #1b7f4a;
    ...
    * {
    ...
    body {
      margin: 0;
      padding: 14px;
      background: linear-gradient(180deg, #e8ecef 0%, #dde3e6 100%);
    ...
    .photo-grid-container .photo-caption {
      font-size: 8px;
      line-height: 1.2;
      font-weight: 500;
      margin-top: 3px;
...
<body>
  ...
  // Carrega checklist salvo do backend via OC quando container é selecionado
  const carregarChecklistDoContainer = async (containerKey) => {
    if (!containerKey) return;
    try {
      const carreg = buscarCarregamentoPorChave(containerKey);
      const oc = carreg?.id || carreg?.PLANCARREG_IN_CODIGO;
      if (!oc) return;
      const res = await api.get('/carregamentos/checklist', { params: { oc: String(oc) } });
      const saved = res?.checklist || res?.data?.checklist;
      if (Array.isArray(saved) && saved.length > 0) {
        setChecklist((current) =>
          current.map((item) => {
            const match = saved.find((s) => s.key === item.key);
            return match && match.value !== null ? { ...item, value: match.value, temperatura: match.temperatura ?? item.temperatura } : item;
          })
        );
        console.log(`[Checklist] Carregado ${saved.filter(i => i.value).length} itens para OC ${oc}`);
      }
    } catch (e) {
      console.warn('[Checklist] Falha ao carregar:', e?.message);
    }
  };

  // Adiciona foto local no Imprimir/Maturação (apenas para PDF, não salva)
  const addImprimirMangaLocal = async (itemKey, fromCamera = false) => {
    try {
      let result;
      if (fromCamera) {
      ...
      const uri = result.assets?.[0]?.uri;
      if (!uri) return;
      setImprimirMangaLocais((prev) => ({
        ...prev,
        [itemKey]: [...(prev[itemKey] || []), uri],
      }));
    } catch (e) {
      console.warn('[ImprimirManga] Falha ao selecionar foto:', e?.message);
    }
  };

  // Upload de foto para pasta do container (Imprimir → Container)
  const uploadFotoContainerImprimir = async (campoKey, fromCamera = false) => {
    if (!containerSelecionadoKey) {
      Alert.alert('Atenção', 'Selecione um container na aba Informações antes de adicionar fotos.');
      return;
    }
    try {
      const carreg = buscarCarregamentoPorChave(containerSelecionadoKey);
      if (!carreg?.apelido) {
        Alert.alert('Atenção', 'Container não encontrado. Selecione um container válido.');
        return;
      }
      let result;
      if (fromCamera) {
      ...
      const formData = new FormData();
      formData.append('apelido', carreg.apelido);
      formData.append('oc', String(carreg.id || ''));
      formData.append('campos_json', JSON.stringify(assets.map(() => campoKey)));
      assets.forEach((asset, idx) => {
        const ext = (asset.uri || '').split('.').pop()?.toLowerCase() || 'jpg';
        formData.append('fotos', { uri: asset.uri, name: `foto_${campoKey}_${idx + 1}.${ext}`, type: 'image/jpeg' });
      });
      await api.post('/priorizacao-pallets/upload-fotos', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
        timeout: 60000,
      });
      carregarFotosContainerImprimir(containerSelecionadoKey);
    } catch (e) {
      console.warn('[ContainerImprimir] Falha ao fazer upload:', e?.message);
      Alert.alert('Erro', 'Não foi possível enviar a foto.');
    }
  };

  // Informacoes | Mangas | Container | Previa PDF | Rascunhos
  ...
  const renderGeneralInfo = () => {
    ...
    <>
        ...
        <View style={st.card}>
          ...
          {(imprimirMangaSection.items || []).slice(0, 4).map((item) => {
            ...
            const serverUrls = temVariedades
              ...
              : allUrls;
            const locais = imprimirMangaLocais[item.key] || [];
            return (
              <View key={`maturacao-pdf-${item.key}`} style={{ marginBottom: 10 }}>
                <ItemCard
                  item={item}
                  onPickGallery={() => addImprimirMangaLocal(item.key, false)}
                  onPickCamera={() => addImprimirMangaLocal(item.key, true)}
                  onPreviewPhoto={(photo) =>
                    setPhotoViewer({
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6, paddingHorizontal: 4 }}>
                    {locais.map((uri, idx) => (
                      <View key={idx} style={{ position: 'relative' }}>
                        <Image source={{ uri }} style={{ width: 70, height: 70, borderRadius: 6 }} resizeMode="cover" />
                        <TouchableOpacity
                          onPress={() => setImprimirMangaLocais((prev) => ({ ...prev, [item.key]: (prev[item.key] || []).filter((_, i) => i !== idx) }))}
                          style={{ position: 'absolute', top: -4, right: -4, backgroundColor: '#C62828', borderRadius: 10, width: 18, height: 18, alignItems: 'center', justifyContent: 'center' }}
                        >
      ...
      {generalInfoTab === 'container' && (<>
        {PRIORIZACAO_FOTO_CAMPOS.filter(
          (c) => !['maturacao_variedade', 'firmeza_variedade'].includes(c.key)
        ).map((campo, idx) => {
          const serverUrls = imprimirContainerFotos.data?.[campo.key] || [];
          return (
            <View key={campo.key} style={[st.card, { marginBottom: 10 }]}>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#222', marginBottom: 8 }}>
                {`${idx + 1}. ${campo.label.replace(/^\d+\.\s*/, '')}`}
              </Text>
              {imprimirContainerFotos.loading ? (
                <ActivityIndicator size="small" color={GREEN} />
              ) : serverUrls.length > 0 ? (
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
                  {serverUrls.map((url, i) => (
                    <Image key={i} source={{ uri: url }} style={{ width: 80, height: 80, borderRadius: 6 }} resizeMode="cover" />
                  ))}
                </View>
              ) : null}
              <View style={{ flexDirection: 'row', gap: 10, marginTop: 6 }}>
                <TouchableOpacity
                  style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: GREEN, borderRadius: 8, paddingVertical: 10 }}
                  onPress={() => uploadFotoContainerImprimir(campo.key, true)}
                  activeOpacity={0.8}
                >
                  <MaterialIcons name="camera-alt" size={18} color="#fff" />
                  <Text style={{ color: '#fff', fontSize: 13, fontWeight: '700' }}>Câmera</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: GREEN, borderRadius: 8, paddingVertical: 10 }}
                  onPress={() => uploadFotoContainerImprimir(campo.key, false)}
                  activeOpacity={0.8}
                >
                  <MaterialIcons name="photo-library" size={18} color="#fff" />
                  <Text style={{ color: '#fff', fontSize: 13, fontWeight: '700' }}>Galeria</Text>
                </TouchableOpacity>
              </View>
            </View>
          );
      ...
      {generalInfoTab === 'previa_pdf' && (
        <View style={{ flex: 1 }}>
          {previaLoading && (
            <View style={{ alignItems: 'center', justifyContent: 'center', padding: 40 }}>
              <ActivityIndicator size="large" color={GREEN} />
              <Text style={{ marginTop: 12, color: '#555', fontSize: 13 }}>Gerando prévia...</Text>
            </View>
          )}
          {!previaLoading && previaHtml && (
            <>
              <View style={{ flexDirection: 'row', justifyContent: 'flex-end', paddingHorizontal: 12, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#DDE', backgroundColor: '#F4F7F2' }}>
                <TouchableOpacity onPress={gerarPreviaEmbarque} activeOpacity={0.8}>
                  <MaterialIcons name="refresh" size={20} color={GREEN} />
                </TouchableOpacity>
              </View>
              <WebView
                source={{ html: previaHtml }}
                style={{ height: 4000 }}
                originWhitelist={['*']}
                javaScriptEnabled
                scrollEnabled={false}
          ...
          {!previaLoading && !previaHtml && (
            <View style={{ alignItems: 'center', justifyContent: 'center', padding: 40 }}>
              ...
              <Text style={{ color: '#999', fontSize: 13, marginTop: 10 }}>Prévia não disponível</Text>
            </View>
          )}
        </View>
      )}
  ...
  return (
    <SafeAreaView style={st.safe}>
      ...
      <View style={st.header}>
        ...
        <TouchableOpacity
          onPress={() => handleGeneralInfoTabPress('rascunhos')}
      ...
      {step === 0 && (
        <View style={{ backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderBottomColor: '#E8EEE8', paddingVertical: 10, paddingHorizontal: 8, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3 }}>
          <GeneralInfoStepDots currentTab={generalInfoTab} onPress={handleGeneralInfoTabPress} />
        </View>
      )}

      {step === 1 && (
        <View style={{ backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderBottomColor: '#E0E0E0', paddingTop: 10, paddingBottom: 14, paddingHorizontal: 12, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#F5F5F5', borderRadius: 10, borderWidth: 1.5, borderColor: '#D0D0D0', paddingHorizontal: 14, minHeight: 52 }}>
            ...
            <TextInput
              style={{ flex: 1, fontSize: 16, color: '#333', paddingVertical: 12 }}
              placeholder="Filtrar por container, apelido ou OC..."
              placeholderTextColor="#BBB"
              value={filtroPriorizacao}
              onChangeText={setFiltroPriorizacao}
              clearButtonMode="while-editing"
            />
            {filtroPriorizacao.length > 0 && (
              <TouchableOpacity onPress={() => setFiltroPriorizacao('')} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      ...
      >
        {isGeneratingPdf ? (
          ...
          <MaterialIcons
            name={step === 0 && generalInfoTab === 'previa_pdf' ? 'send' : step === TOP_FLOW_LAST_INDEX ? 'send' : 'arrow-forward'}
            size={30}
            color="#FFFFFF"
          />
        )}
      ...
      >
        <Pressable style={st.overlay} onPress={() => { setContainerSearch(''); setShowContainerModal(false); }}>
          <Pressable style={st.varModalBox} onPress={(e) => e.stopPropagation()}>
            <View style={st.varModalHeader}>
              <Text style={st.varModalTitle}>Selecionar Container</Text>
              <TouchableOpacity style={st.closeCircle} onPress={() => { setContainerSearch(''); setShowContainerModal(false); }}>
                <MaterialIcons name="close" size={20} color="#333" />
              </TouchableOpacity>
            </View>
            <TextInput
              style={st.clienteSearchInput}
              placeholder="Filtrar por container ou nome"
            ...
            />
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              {containersHistoricoLoading && (
                <View style={{ alignItems: 'center', paddingVertical: 20 }}>
                  <ActivityIndicator size="small" color={GREEN} />
                </View>
              )}
              {!containersHistoricoLoading && filteredContainers.map((carreg) => {
              ...
              {!containersHistoricoLoading && filteredContainers.length === 0 && (
                <View style={st.emptyDraftBox}>
                  <Text style={st.emptyDraftText}>Nenhum container encontrado no histórico</Text>
                </View>
              )}
            </ScrollView>
          </Pressable>
      ...
      <Modal
        ...
        transparent={false}
        animationType="slide"
        onRequestClose={fecharModalCarregamento}
      >
        <View style={{ flex: 1, backgroundColor: '#F2F2F2' }}>
          <StatusBar barStyle="dark-content" backgroundColor="#FFFFFF" />
          {/* Safe area spacer */}
          ...
          {/* Header com logo */}
          <View style={st.header}>
            <TouchableOpacity onPress={fecharModalCarregamento} style={st.backBtn}>
              <MaterialIcons name="arrow-back" size={24} color={GREEN} />
            </TouchableOpacity>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <Image source={require('../../../assets/logoagrodann.png')} style={st.logo} resizeMode="contain" />
          ...
          <>
            ...
            <View style={{ backgroundColor: '#FFF', borderBottomWidth: 1, borderBottomColor: '#E0E0E0', paddingTop: 10, paddingBottom: 14, paddingHorizontal: 12 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#F5F5F5', borderRadius: 10, borderWidth: 1.5, borderColor: '#D0D0D0', paddingHorizontal: 14, minHeight: 52 }}>
                ...
                <TextInput
                  style={{ flex: 1, fontSize: 16, color: '#333', paddingVertical: 12 }}
                  placeholder="Filtrar por pallet, variedade ou controle..."
                  placeholderTextColor="#BBB"
                  value={filtroPallets}
                  onChangeText={setFiltroPallets}
                  clearButtonMode="while-editing"
                />
                {filtroPallets.length > 0 && (
                  <TouchableOpacity onPress={() => setFiltroPallets('')} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                    <MaterialIcons name="close" size={16} color="#999" />
                  </TouchableOpacity>
                )}
              </View>
            </View>
          <ScrollView ref={modalScrollViewRef} style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
            {/* Info resumo */}
            <View style={pgSt.modalInfoBar}>
              ...
              {!!selectedCarregamento?.motorista && (
                <Text style={pgSt.modalSubtitle}>Motorista: {selectedCarregamento.motorista}</Text>
              )}
            </View>

            {(selectedCarregamento?.pallets || []).map((p, originalIdx) => ({ p, originalIdx })).filter(({ p }) => {
              const q = filtroPallets.trim().toLowerCase();
              if (!q) return true;
              return (
                String(p.palletId || '').toLowerCase().includes(q) ||
                (p.variedade || '').toLowerCase().includes(q) ||
                String(p.controle || '').toLowerCase().includes(q) ||
              ...
              return (
                <View key={`cmodal-${p.palletId}-${originalIdx}`} style={pgSt.pCard}>
                  <View style={pgSt.pTopRow}>
                    ...
                    <View style={{ alignItems: 'center' }}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => toggleCarregPalletEtiqueta(originalIdx)}
                        activeOpacity={0.7}
                      >
                  ...
                  <View style={pgSt.pTempRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <TextInput
                        ...
                        value={p.temp1}
                        onChangeText={(text) => setCarregPalletTemp(originalIdx, 'temp1', text)}
                        onFocus={(e) => {
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <TextInput
                        style={pgSt.pTempInput}
                        value={p.temp2}
                        onChangeText={(text) => setCarregPalletTemp(originalIdx, 'temp2', text)}
                        onFocus={(e) => {
                          const target = e?.target ?? e?.nativeEvent?.target;
                          if (target && modalScrollViewRef.current?.scrollResponderScrollNativeHandleToKeyboard) {
                            setTimeout(() => modalScrollViewRef.current.scrollResponderScrollNativeHandleToKeyboard(target, 320, true), 80);
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
            ...
            {[...PRIORIZACAO_FOTO_CAMPOS].sort((a, b) => {
              const aTemFoto = (servidorFotosInline[a.key]?.urls?.length || 0) > 0;
              const bTemFoto = (servidorFotosInline[b.key]?.urls?.length || 0) > 0;
              if (aTemFoto === bTemFoto) return 0;
              return aTemFoto ? 1 : -1;
            }).map((campo) => {
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
            <View style={st.card}>
              <Text style={st.cardTitle}>Container</Text>

              {/* Campos Termógrafo e Etiqueta */}
              <View style={{ flexDirection: 'row', gap: 12, marginBottom: 16 }}>
                <View style={{ flex: 1 }}>
                  <Text style={st.fieldLabel}>Nº Termógrafo</Text>
                  <TextInput
                    style={[st.inputBox, { paddingVertical: 12 }]}
                    value={modalContainerTermografo}
                    onChangeText={setModalContainerTermografo}
                ...
                <View style={{ flex: 1 }}>
                  <Text style={st.fieldLabel}>Nº Etiqueta</Text>
                  <TextInput
                    style={[st.inputBox, { paddingVertical: 12 }]}
                    value={modalContainerEtiqueta}
                    onChangeText={setModalContainerEtiqueta}
                    placeholder="Digite o número"
                    placeholderTextColor="#C0C0C0"
                    keyboardType="default"
                  />
              ...
              {modalChecklist.map((item) => {
                const labelPos = item.usaSimNao ? 'Sim' : 'Conforme';
                const labelNeg = item.usaSimNao ? 'Não' : 'Não Conforme';
