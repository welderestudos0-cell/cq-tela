...
function getWeekNumber(dateStr) {
  try {
    ...
    // Ajusta para o próximo Thursday
    date.setDate(date.getDate() + 4 - ((date.getDay() || 7) - 1));
    const yearStart = new Date(date.getFullYear(), 0, 1);
    yearStart.setDate(yearStart.getDate() - ((yearStart.getDay() || 7) - 1) + 3);
...
// Converte uma string de valor decimal (com vírgula ou ponto) para número, retornando 0.0 se vazio/inválido.
...
function normalizeCriterioKey(value = '') {
  return String(value || '')
    .trim()
    .normalize('NFD').replace(/[̀-ͯ]/g, '')
    .replace(/[^A-Za-z0-9]/g, '')
...
function isBrixCriterio(value = '') {
  return normalizeCriterioKey(value).includes('BRIX');
}

function formatCriterioLabel(value = '') {
  return isBrixCriterio(value) ? '°Brix' : value;
}
...
export default function AnaliseFrutos({ navigation }) {
  ...
  const criteriosLoteOptions = isShelfLife
    ? [...CRITERIOS_BASE, 'Peso (g)']
    : isPreColheita
      ? ['°Brix', 'Matéria Seca', 'Maturação', 'Penetrometria']
      : (isProducao || isAcompanhamento)
        ? CRITERIOS_PRE_COLHEITA
        : CRITERIOS_LOTE;
  ...
  const saveOfflineAnaliseFrutos = async (payload) => {
    ...
    // Remove fotos (dados muito pesados) para salvar apenas a estrutura
    const payloadSemFotos = {
      ...payload,
      fotos: [],
      fotos_salvas: [],
      fotos_producao: null,
      fotos_producao_salvas: null,
    };
    ...
    const offlineItem = {
      ...
      fazenda: payloadSemFotos?.fazenda_talhao || payloadSemFotos?.fazenda || '',
      talhao: payloadSemFotos?.talhao || '',
      usuario: payloadSemFotos?.avaliador || String(user?.nome || user?.name || '').trim(),
      matricula: String(user?.matricula || '').trim() || 'Não Informada',
      timestamp: nowIso,
      dataColeta: payloadSemFotos?.data || nowIso,
      sincronizado: false,
      _syncStatus: 'pending',
    ...
    try {
      ...
      console.log('[AnaliseFrutos] Salvo offline com sucesso:', offlineId);
      return offlineItem;
    } catch (e) {
      console.error('[AnaliseFrutos] Erro ao salvar offline:', e?.message);
      throw new Error(`Falha ao salvar no cache: ${e?.message}`);
    }
  };
  ...
  const getLoteValue = (fruit, criterio) => {
    const valores = fruit?.valores_lotes || {};
    if (isBrixCriterio(criterio)) {
      const brixValue = valores['°Brix'] ?? valores.Brix;
      return brixValue != null ? String(brixValue) : '';
    }
    const value = valores[criterio];
  ...
  const updateLoteValue = (fruitIndex, criterio, value) => {
    setFruits((previous) => previous.map((fruit, i) => (i === fruitIndex ? {
      ...
      valores_lotes: {
        ...(fruit.valores_lotes || {}),
        [criterio]: value,
        ...(isBrixCriterio(criterio) ? { Brix: value, '°Brix': value } : {}),
      },
    } : fruit)));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        ...
        Alert.alert('Salvo offline', 'Seus dados foram salvos no sininho. Sincronize quando tiver internet.', [
          { text: 'OK' },
        ]);
      } catch (offlineError) {
        console.error('[AnaliseFrutos] Falha ao salvar offline:', offlineError?.message);
        Alert.alert('Falha ao salvar', `Não foi possível salvar: ${offlineError?.message || 'Erro desconhecido'}`);
      }
    } finally {
      setSaving(false);
    }
...
<body>
  ...
  // Rola o ScrollView para manter o campo focado acima do teclado (com reforco no Android).
  const handleInputFocus = (event, isLoteField = false) => {
    ...
    const baseOffset = isLoteField ? -400 : 110;
    const keyboardOffset = keyboardHeight > 0 ? Math.floor(keyboardHeight * 0.35) : 0;
    const extraOffset = baseOffset + keyboardOffset;
    const keepPositiveOffset = !isLoteField;

    const scrollFocusedField = () => {
      scrollViewRef.current?.scrollResponderScrollNativeHandleToKeyboard(target, Math.abs(extraOffset), keepPositiveOffset);
    };
    ...
    // Segunda passada apos abertura do teclado para evitar o campo ficar escondido.
    setTimeout(() => {
      scrollFocusedField();
    }, 320);
  };
                    ...
                    <View key={`batch-row-${row.numero_fruto}-${row.criterio}`}>
                      ...
                      <View style={[styles.batchDataRow, rowIndex % 2 === 0 ? styles.batchRowOdd : styles.batchRowEven]}>
                        <Text style={[styles.batchTextCell, isMat ? styles.batchColCriterio : styles.batchColCriterioNoDanos, styles.batchCellBorderRight, styles.batchCriterionText]}>
                          {formatCriterioLabel(row.criterio)}
                        </Text>
                        ...
                        <View style={isMat ? styles.batchColValor : styles.batchColValorNoDanos}>
                          <TextInput
                            ...
                            onFocus={(event) => handleInputFocus(event, true)}
      ...
      >
        <Pressable style={styles.modalOverlay} onPress={() => setShowOfflineSavedModal(false)}>
          <Pressable style={[styles.modalCard, styles.offlineSavedModalCard]} onPress={() => {}}>
            <MaterialIcons name="wifi-off" size={32} color="#C56A17" style={{ alignSelf: 'center', marginBottom: 12 }} />

            <Text style={styles.offlineSavedTitle}>Sem conexão Wi-Fi</Text>

            <Text style={styles.offlineSavedText}>
              A análise foi salva no sininho. Quando conectar ao Wi-Fi, sincronize na tela inicial.
            </Text>

            <View style={styles.offlineSavedActions}>
              <TouchableOpacity
                style={styles.offlineSavedButton}
                onPress={() => {
                ...
                activeOpacity={0.8}
              >
                <Text style={styles.offlineSavedButtonText}>Ok, entendi</Text>
              </TouchableOpacity>
            </View>
          </Pressable>
        </Pressable>
                    ...
                    <View style={styles.historySection}>
                      ...
                      {lotes.slice(0, 30).map((lote, idx) => (
                        <Text key={`lote-${idx}`} style={styles.historyLineSmall}>
                          Fruto {lote?.numero_fruto ?? '-'} | {formatCriterioLabel(lote?.criterio || '-')}: {lote?.valor ?? '-'} {lote?.danos_internos ? `| Danos: ${lote.danos_internos}` : ''}
                        </Text>
                      ))}
...
const styles = StyleSheet.create({
  ...
  offlineSavedModalCard: { maxWidth: 340, paddingHorizontal: 24, paddingTop: 28, paddingBottom: 22 },
  offlineSavedTitle: {
    fontSize: 16,
    fontWeight: '700',
    color: '#1A1A1A',
  ...
  offlineSavedButton: {
    backgroundColor: '#0B8A43',
    borderRadius: 8,
    paddingVertical: 12,
    alignItems: 'center',
  },
  offlineSavedButtonText: {
    fontSize: 14,
    fontWeight: '700',
    color: '#fff',
  },
