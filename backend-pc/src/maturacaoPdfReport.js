...
let PNG = null;
try {
  const pngModule = await import('png-js');
...
const drawPdfText = (ops, {
  ...
  ops.push(`(${escapePdfLatin1(String(text ?? ''))}) Tj`);
  ops.push('ET');
};
...
const readJpegDimensions = (buffer) => {
  ...
  while (offset < buffer.length) {
    ...
    const dataStart = segmentStart + 2;
    const isSofMarker =
...
const rgbaToRgbBuffer = (rgba, width, height) => {
  const totalPixels = Math.max(1, width * height);
  const rgb = Buffer.alloc(totalPixels * 3);
  let src = 0;
  let dst = 0;

  while (src < rgba.length && dst < rgb.length) {
    const r = rgba[src++] ?? 0;
    const g = rgba[src++] ?? 0;
    const b = rgba[src++] ?? 0;
    const a = rgba[src++] ?? 255;
    ...
    rgb[dst++] = Math.round((g * alpha) + (255 * (1 - alpha)));
    rgb[dst++] = Math.round((b * alpha) + (255 * (1 - alpha)));
  }

  return rgb;
};

const decodePngPixels = async (uri) => {
  if (!PNG) {
    throw new Error('png-js indisponivel');
  }

  const png = PNG.load(uri);
  const pixels = await new Promise((resolve, reject) => {
    try {
      png.decode((decodedPixels) => resolve(Buffer.from(decodedPixels)));
    } catch (error) {
      reject(error);
    }
  });
...
const prepareNodePhotoAsset = async (uri) => {
  const arquivo = String(uri || '').trim();
  if (!arquivo) {
    return { supported: false, reason: 'Caminho da foto ausente' };
  }

  if (!fs.existsSync(arquivo)) {
    return { supported: false, reason: `Foto nao encontrada: ${arquivo}` };
  }
  ...
  if (extensao === '.jpg' || extensao === '.jpeg') {
    ...
    if (!dimensions) {
      return { supported: false, reason: 'JPEG invalido' };
    }
  ...
  if (extensao === '.png') {
    const decoded = await decodePngPixels(arquivo);
    const rgb = rgbaToRgbBuffer(decoded.pixels, decoded.width, decoded.height);
    ...
    return {
      ...
      hex: compressed.toString('hex').toUpperCase(),
    };
  }

  if (extensao === '.webp') {
    return { supported: false, reason: 'WebP nao suportado no gerador do PDF' };
  }

  return { supported: false, reason: `Formato de imagem nao suportado: ${extensao || 'desconhecido'}` };
};
...
const preparePdfPhotoAsset = async (uri) => {
  try {
    ...
    return await prepareNodePhotoAsset(uri);
  } catch (error) {
...
const preparePdfLogoAsset = async () => {
  try {
    if (!fs.existsSync(LOGO_PATH)) return { supported: false };
    return await prepareNodePhotoAsset(LOGO_PATH);
  } catch {
...
const drawDamageDistributionChart = (ops, { topY, data, total }) => {
  ...
  const categoriasBase = [
    { label: 'Tecido Esponjoso', count: sumPdfInt(data.te) },
...
const getEvaluationDataset = (data) => {
  const evalItems = [
    ['Tecido Esponjoso', String(sumPdfInt(data.te))],
...
const estimateEvaluationSummaryBlockHeight = (data) => {
  ...
  const chartHeight = (chartRows * 21) + 124;
...
const buildEvaluationPaginationPlan = ({ data, coverAvailable, continuationAvailable }) => {
  ...
  const diagnosticRows = rowsToRender.map(([label, count], index) => ({
    label,
  ...
  if (continuationCapacityWithSummary <= 0) {
    return {
      diagnosticRows,
      totalRowNumber,
      coverSegment: {
        startIndex: 0,
        count: Math.max(0, Math.min(coverCapacityNoSummary, diagnosticRows.length)),
        includeTotalRow: false,
        includeSummaryPanel: false,
      },
      continuationSegments: [
        {
          startIndex: Math.max(0, Math.min(coverCapacityNoSummary, diagnosticRows.length)),
          count: Math.max(0, diagnosticRows.length - Math.max(0, Math.min(coverCapacityNoSummary, diagnosticRows.length))),
          includeTotalRow: true,
          includeSummaryPanel: true,
        },
      ],
...
const buildPdfDocument = (pageModels, photoAssetMap, logoAsset) => {
  ...
  const fontTitleObj = allocateObject();
  ...
  // Evita fonte customizada sem embed (invisivel em alguns visualizadores iOS).
  objects[fontTitleObj - 1] = '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>';

  if (logoObjNumber && logoAsset?.supported) {
    const logoFilter = logoAsset.kind === 'png'
    ...
    objects[logoObjNumber - 1] =
      ...
      `/ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter ${logoFilter} /Length ${logoAsset.hex.length + 1} >>\n` +
      `stream\n${logoAsset.hex}>\nendstream`;
  }

  loadedImageEntries.forEach(([index, asset]) => {
    const objectNumber = imageObjectNumbers.get(index);
    const filter = asset.kind === 'png'
      ? '[/ASCIIHexDecode /FlateDecode]'
      : '[/ASCIIHexDecode /DCTDecode]';

    objects[objectNumber - 1] =
      `<< /Type /XObject /Subtype /Image /Width ${asset.width} /Height ${asset.height} ` +
      `/ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter ${filter} /Length ${asset.hex.length + 1} >>\n` +
      `stream\n${asset.hex}>\nendstream`;
  });
  ...
  pageModels.forEach((pageModel, index) => {
    ...
    objects[pageObj - 1] =
      ...
      `/Resources << /Font << /F1 ${fontRegularObj} 0 R /F2 ${fontBoldObj} 0 R /F3 ${fontLabelObj} 0 R /F4 ${fontTitleObj} 0 R >>${imageResources} >> ` +
